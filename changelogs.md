#2.2.4
- **Item pickers now filter by category and sort by rarity** -- the item picker dialogs (add item to inventory, bulk player item management, guild item actions) have a new type dropdown (Food, Accessory, Weapon, Armor, Material, etc.) next to the search box. Ammo, schematics, and special weapons now have proper categories too (they previously had none), with a Misc fallback for anything uncategorized. The add-item pickers also gained a sort dropdown: default order, name A-Z, or rarity high-to-low. Filters, sort, and text search all combine.
- **Passive skill picker tier filter and effect search** -- when adding passives to pals, new tier chips (All / Common / Rare / Epic / Negative) filter the list by passive rank, combined with the text search. A Name/Effect toggle next to the search box switches between searching skill names and what the skill actually does (e.g. typing "work speed" finds Artisan, Serious, etc.). Chips and toggle are color-coded and only appear for passives, not active skills.
- **Edit Quantity no longer caps at 99** -- editing the quantity of an item in a base chest showed 99 instead of the actual count for stacks above 99. The dialog's value was set before its range, so Qt clamped it to the default 0-99 range. The range is now set first (and always covers the current stack size), so the spinner opens at the real amount. Same latent ordering fixed in the effigy quantity prompts and the remove-percentage dialog.
...
- **Fixed use-after-free crash when editing inventory** — every time a dialog was used (add item, edit quantity, pick item, etc.), Python's garbage collector could destroy the dialog's C++ widgets while Qt still had pending events for them, causing a segfault. Globally patched `QObject.deleteLater()` to keep Python wrappers alive until Qt actually destroys the object. Dialog exec results are also kept alive briefly to prevent teardown events hitting freed memory.
