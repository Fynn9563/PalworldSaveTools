#2.1.5
- **Item pickers now filter by category and sort by rarity** — the item picker dialogs (add item to inventory, bulk player item management, guild item actions) have a new type dropdown (Food, Accessory, Weapon, Armor, Material, etc.) next to the search box. The add-item pickers also gained a sort dropdown: default order, name A-Z, or rarity high-to-low. Filters, sort, and text search all combine.
